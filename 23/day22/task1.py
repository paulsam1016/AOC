...
def print_faces(grid):
    ...
    for z in range(max_z + 1):
        if not all(grid[z][yy][xx] == '.' for yy in range(max_y + 1) for xx in range(max_x + 1)):
...
grid = [[['.' if z != 0 else '-' for _ in range(max_x + 1)] for _ in range(max_y + 1)] for z in range(max_z + 1)]

# print_faces(grid)

drop(stack)

for index, (x1, y1, z1, x2, y2, z2) in enumerate(stack):
    for z in range(z1, z2 + 1):
        for y in range(y1, y2 + 1):
            for x in range(x1, x2 + 1):
                grid[z][y][x] = str(index) if grid[z][y][x] == '.' else '?'
